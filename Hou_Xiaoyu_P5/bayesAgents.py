...
def constructBayesNet(gameState):
    ...
    for house in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(house):
            obs_varible = OBS_VAR_TEMPLATE % wall
            
            obsVars.append(obs_varible)
            edges.append((FOOD_HOUSE_VAR,obs_varible))
            edges.append((GHOST_HOUSE_VAR,obs_varible))
            variableDomainsDict[obs_varible] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]

    #add values into variableDomainsDict
...
def fillObsCPT(bayesNet, gameState):
    ...
    for house in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(house):
            obs_value = OBS_VAR_TEMPLATE % wall
            obsFactor = bn.Factor([obs_value], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())

            bleft_check_a = abs(wall[0] - bottomLeftPos[0])
            bleft_check_b = abs(wall[1] - bottomLeftPos[1])
            bleft_check = (bleft_check_a <= 1) and (bleft_check_b <= 1)
            
            tleft_check_a = abs(wall[0] - topLeftPos[0])
            tleft_check_b = abs(wall[1] - topLeftPos[1])
            tleft_check = (tleft_check_a <= 1) and (tleft_check_b <= 1)

            bright_check_a = abs(wall[0] - bottomRightPos[0])
            bright_check_b = abs(wall[1] - bottomRightPos[1])
            bright_check = (bright_check_a <= 1) and (bright_check_b <= 1)

            tright_check_a = abs(wall[0] - topRightPos[0])
            tright_check_b = abs(wall[1] - topRightPos[1])
            tright_check = (tright_check_a <= 1) and (tright_check_b <= 1)

            if bleft_check == True:
                pos = BOTTOM_LEFT_VAL
            elif tleft_check == True:
                pos = TOP_LEFT_VAL
            elif bright_check == True:
                pos = BOTTOM_RIGHT_VAL
            elif tright_check == True:
                pos = TOP_RIGHT_VAL
            else:
                pos = None

            for i in obsFactor.getAllPossibleAssignmentDicts():
                prob = 0

                if i[FOOD_HOUSE_VAR] == pos:
                    if i[obs_value] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif i[obs_value] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                    else:
                        prob = 0
                elif i[GHOST_HOUSE_VAR] == pos:
                    if i[obs_value] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif i[obs_value] == BLUE_OBS_VAL:
                    ...
                    if i[obs_value] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0
                obsFactor.setProbability(i, prob)
            
            bayesNet.setCPT(obs_value, obsFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    #util.raiseNotDefined()
    queryvars = ['foodHouse', 'ghostHouse']
    factor = inference.inferenceByVariableElimination(bayesNet, queryvars, evidence, eliminationOrder)
    prob = 0
    result = dict()
    for assign in factor.getAllPossibleAssignmentDicts():
        new_prob = factor.getProbability(assign)
        if prob < new_prob:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        var = inference.inferenceByVariableElimination(self.bayesNet, queryvars, evidence, eliminationOrder)
        ...
        left_prob = var.getProbability(left)
        right_prob = var.getProbability(right)

        leftExpectedValue = left_prob * WON_GAME_REWARD + right_prob * GHOST_COLLISION_REWARD
        rightExpectedValue = left_prob * GHOST_COLLISION_REWARD + right_prob * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        for prob, explore_evidence in self.getExplorationProbsAndOutcomes(evidence):
            var = self.computeEnterValues(explore_evidence, enterEliminationOrder)
            var1 = max(var)
            expectedValue += prob * var1

...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for iteration in range(0, self.iterations):
            ...
            for state in self.mdp.getStates():
                if (self.mdp.isTerminal(state)):
                    ...
                    next_value[state] = max([self.computeQValueFromValues(state, action) for action in actions])
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for T in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            value = value + T[1] * (R + (self.discount * self.values[T[0]]))
        return value
    ...
    def computeActionFromValues(self, state):
        ...
        next_action = None
        for a in action:
            new_value = self.computeQValueFromValues(state, a)
            if new_value > values:
                values = new_value
                next_action = a
        return next_action

...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        max_action = []
        max_action = [action for action in actions if self.getQValue(state, action) == max_value]
        choice = random.choice(max_action)
        return choice
        #util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.q_value[(state, action)] = q_state + (self.alpha * (reward + (self.discount * q_next_state) - q_state))
        #util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        value = 0
        for feature in F:
          value = value + (self.weights[feature] * F[feature])
        return value 
        #util.raiseNotDefined()

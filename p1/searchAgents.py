...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if position in self.corners:
            if position not in corner:
                corner.append(position)
            if (len(corner)==4):
                return True
            else:
                return False
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corner = set()

    for corner in corners:
        if corner not in visited_corner:
            unvisited_corner.add(corner)
    ...
    while (len(unvisited_corner) != 0):
        ...
        unvisited_corner.remove(min_corner)
        position = min_corner
...
def foodHeuristic(state, problem):
    ...
    for a in positions:
        for b in positions:
            distance = util.manhattanDistance(a,b)
            if distance > result:
                result = distance
    return result
